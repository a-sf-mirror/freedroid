...
save_uint8_t("SpecialForce", &(target->SpecialForce));
...
save_uint8_t("will_rush_tux", &(target->will_rush_tux));
...
save_uint8_t("has_been_taken_over", &(target->has_been_taken_over));
save_uint8_t("follow_tux", &(target->follow_tux));
save_uint8_t("has_greeted_influencer", &(target->has_greeted_influencer));
...
save_uint8_t("attack_target_type", &(target->attack_target_type));
...
save_uint8_t("bot_stuck_in_wall_at_previous_check", &(target->bot_stuck_in_wall_at_previous_check));
...
read_uint8_t(pos, "SpecialForce",  &(target->SpecialForce));
...
read_uint8_t(pos, "will_rush_tux",  &(target->will_rush_tux));
...
read_uint8_t(pos, "has_been_taken_over",  &(target->has_been_taken_over));
read_uint8_t(pos, "follow_tux",  &(target->follow_tux));
read_uint8_t(pos, "has_greeted_influencer",  &(target->has_greeted_influencer));
...
read_uint8_t(pos, "attack_target_type",  &(target->attack_target_type));
...
read_uint8_t(pos, "bot_stuck_in_wall_at_previous_check",  &(target->bot_stuck_in_wall_at_previous_check));
...
autostr_append(savestruct_autostr, "<%s>\n",tag);
save_int16_t("type", &(target->type));
save_int32_t("phase", &(target->phase));
save_uint8_t("mine", &(target->mine));
save_gps("pos", &(target->pos));
...
save_uint8_t("pass_through_hit_bodies", &(target->pass_through_hit_bodies));
...
save_int32_t("faction", &(target->faction));
save_uint8_t("hit_type", &(target->hit_type));
autostr_append(savestruct_autostr, "</%s>\n", tag);
return 0;
}
...
int read_bullet(char* buffer, char * tag, bullet * target)
{
		...
		*epos = 0;
		read_int16_t(pos, "type",  &(target->type));
read_int32_t(pos, "phase",  &(target->phase));
read_uint8_t(pos, "mine",  &(target->mine));
read_gps(pos, "pos",  &(target->pos));
...
read_uint8_t(pos, "pass_through_hit_bodies",  &(target->pass_through_hit_bodies));
...
read_int32_t(pos, "faction",  &(target->faction));
read_uint8_t(pos, "hit_type",  &(target->hit_type));
*epos = '>'; 
return 0;
}
...
save_uint8_t("chat_character_initialized", &(target->chat_character_initialized));
save_uint8_t_array("chat_flags", (target->chat_flags), MAX_ANSWERS_PER_PERSON);
...
save_item_dynarray("npc_inventory", &(target->npc_inventory));
...
read_uint8_t(pos, "chat_character_initialized",  &(target->chat_character_initialized));
read_uint8_t_array(pos, "chat_flags",  (target->chat_flags), MAX_ANSWERS_PER_PERSON);
...
read_item_dynarray(pos, "npc_inventory",  &(target->npc_inventory));
...
save_uint8_t("god_mode", &(target->god_mode));
...
save_string("savegame_version_string", &(target->savegame_version_string));
save_int32_t("base_cooling", &(target->base_cooling));
save_int32_t("base_dexterity", &(target->base_dexterity));
save_int32_t("base_physique", &(target->base_physique));
save_int32_t("base_strength", &(target->base_strength));
save_int32_t("cooling", &(target->cooling));
save_int32_t("dexterity", &(target->dexterity));
save_int32_t("physique", &(target->physique));
...
save_uint8_t_array("HaveBeenToLevel", (target->HaveBeenToLevel), MAX_LEVELS);
...
read_uint8_t(pos, "god_mode",  &(target->god_mode));
...
read_string(pos, "savegame_version_string",  &(target->savegame_version_string));
read_int32_t(pos, "base_cooling",  &(target->base_cooling));
read_int32_t(pos, "base_dexterity",  &(target->base_dexterity));
read_int32_t(pos, "base_physique",  &(target->base_physique));
read_int32_t(pos, "base_strength",  &(target->base_strength));
read_int32_t(pos, "cooling",  &(target->cooling));
read_int32_t(pos, "dexterity",  &(target->dexterity));
read_int32_t(pos, "physique",  &(target->physique));
...
read_uint8_t_array(pos, "HaveBeenToLevel",  (target->HaveBeenToLevel), MAX_LEVELS);
...
save_int32_t("type", &(target->type));
save_int32_t("max_durability", &(target->max_durability));
save_float("current_durability", &(target->current_durability));
...
read_int32_t(pos, "type",  &(target->type));
read_int32_t(pos, "max_durability",  &(target->max_durability));
read_float(pos, "current_durability",  &(target->current_durability));
...
save_int32_t("cheat_double_speed", &(target->cheat_double_speed));
...
save_int32_t("show_all_floor_layers", &(target->show_all_floor_layers));
...
read_int32_t(pos, "cheat_double_speed",  &(target->cheat_double_speed));
...
read_int32_t(pos, "show_all_floor_layers",  &(target->show_all_floor_layers));
