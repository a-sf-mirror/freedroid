...
int read_mission(char* buffer, char * tag, mission * target)
{
		...
		if ( ! epos ) return 2;
		*epos = 0;
		read_string(pos, "MissionName", (char*) &(target->MissionName));
...
read_string(pos, "savegame_version_string", (char*) &(target->savegame_version_string));
...
read_string(pos, "character_name", (char*) &(target->character_name));
