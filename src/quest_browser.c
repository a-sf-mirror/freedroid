...
{
	struct auto_string *stats_buffer = alloc_autostr(5000);
	...
	//Assemble Output
	autostr_append(stats_buffer, _("\1Parafunken Statistics\2\n"));

	// This is where we display grand total statistics
	autostr_append(stats_buffer, _("\1Overview\2\n"));
	statistics_browser_lines_needed[display] = get_lines_needed(stats_buffer->value, mission_description_rect, 1);
	if (stats_display[display++]) {
		autostr_append(stats_buffer, _("Distance Traveled: \3%.1fm\2\n"), Me.meters_traveled);
		autostr_append(stats_buffer, _("Destroyed Enemies: \3%i\2\n"), total_destroyed);
		autostr_append(stats_buffer, _("Captured Enemies: \3%i\2\n"), total_takeover_success);
		autostr_append(stats_buffer, _("Damage Dealt: \3%i\2\n"), total_damage_dealt);
	}
	...
	autostr_append(stats_buffer, _("\1Preferences\2\n"));
	statistics_browser_lines_needed[display] = get_lines_needed(stats_buffer->value, mission_description_rect, 1);
	if (stats_display[display++]) {
		if (fav_destroyed != -1) {
			autostr_append(stats_buffer, _("Attacking: \3%s\2 (\3%i\2 "), 
				Druidmap[fav_destroyed].default_short_description, Me.destroyed_bots[fav_destroyed]);
			if (Druidmap[fav_destroyed].is_human)
				autostr_append(stats_buffer, _("killed,"));
			else
				autostr_append(stats_buffer, _("destroyed,"));
			autostr_append(stats_buffer, _(" \3%i\2 DP)\n"), Me.damage_dealt[fav_destroyed]);
		} else
			autostr_append(stats_buffer, _("Attacking: no droids destroyed\n"));

		if (fav_capture_target != -1)
			autostr_append(stats_buffer, _("Takeover: \3%s\2 (\3%i%%\2 rate, \3%i\2 attempts)\n"),
			...
			autostr_append(stats_buffer, _("Takeover: none successful\n"));


		if (worst_capture_target != -1)
			autostr_append(stats_buffer, _("Failed Takeover: \3%s\2 (\3%i%%\2 rate, \3%i\2 attempts)\n"),
			...
			autostr_append(stats_buffer, _("Failed Takeover: no failures\n"));
	}
	...
	for (counter = 0; counter < Number_Of_Droid_Types; counter++) {
		...
		if (Me.damage_dealt[model] || Me.TakeoverSuccesses[model] || Me.TakeoverFailures[model] || Me.destroyed_bots[model]) {
			autostr_append(stats_buffer, "\1%s\2\n", Druidmap[model].default_short_description);
			statistics_browser_lines_needed[display] = get_lines_needed(stats_buffer->value, mission_description_rect, 1);
			if (stats_display[display++]) {
				if (Druidmap[model].is_human) {
					autostr_append(stats_buffer, _("Killed: \3%d\2\n"), Me.destroyed_bots[model]);
				} else {
					autostr_append(stats_buffer, _("Destroyed: \3%d\2\n"), Me.destroyed_bots[model]);
					autostr_append(stats_buffer, _("Takeovers: \3%d\2/\3%d\2/\3"), Me.TakeoverSuccesses[model], Me.TakeoverFailures[model]);
					if (Me.TakeoverSuccesses[model] + Me.TakeoverFailures[model])
						autostr_append(stats_buffer, _("%d%%\2"),
							(100 * Me.TakeoverSuccesses[model])/(Me.TakeoverSuccesses[model] + Me.TakeoverFailures[model]));
					else
						autostr_append(stats_buffer, _(" 0%%\2"));
					autostr_append(stats_buffer, _(" (success/fail/ratio)\n"));
				}
				autostr_append(stats_buffer, _("Damage: \3%d\2 (dealt)\n"), Me.damage_dealt[model]);
			}
		} else {
	...
	DisplayText(stats_buffer->value, mission_description_rect.x,
...
static void quest_browser_display_mission_list(int list_type)
{
	...
	if (something_was_displayed) {
		...
		for (mis_num = 0; mis_num < MAX_MISSIONS_IN_GAME; mis_num++) {
			...
			if (rect_short->y <= mission_description_rect.y - 4)
				continue;
			if (rect_short->y >= mission_description_rect.y + mission_description_rect.h - FontHeight(GetCurrentFont()) * TEXT_STRETCH)
				continue;
	...
	} else if (list_type == QUEST_BROWSER_SHOW_NOTES) {
		print_statistics();
	} else {
		const char *txt;
