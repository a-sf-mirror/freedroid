...
int DoSkill(int skill_index, int SpellCost)
{
	...
	case PROGRAM_FORM_BULLET:
		...
		if (SpellSkillMap[skill_index].graphics_code != -1)
			...
			FillInDefaultBulletStruct(&bul_parms, GetBulletByName("half_magenta"), GetItemIndexByName("Laser pistol"));
...
#define FIRST_SKILLRECT_Y 16
	...
	for (i = 0; i < NUMBER_OF_SKILLS_PER_SKILL_PAGE; i++) {
		...
		if (sci != 10) {
			...
			char str[10];
			sprintf(str, "F%d\n", 5 + sci);
			display_text(str, ButtonRect.x + ButtonRect.w - 2 - TextWidth(str), ButtonRect.y, &SkillScreenRect);
		}
		// Name of the skill
