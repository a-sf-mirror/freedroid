...
EXTERN int smashable_barrel_below_mouse_cursor ( ) ;
...
EXTERN void correct_tux_position_according_to_jump_thresholds ( );
EXTERN int isignf (float);
EXTERN void InitInfluPositionHistory( );
...
EXTERN void FillInDefaultBulletStruct ( bullet * CurBullet, int bullet_image_type, int weapon_item_type );
EXTERN void FireTuxRangedWeaponRaw ( int weapon_item_type , int bullet_image_type , bullet *, moderately_finepoint target_location ) ;
EXTERN void move_tux ( ) ;
EXTERN void animate_tux ( ) ;
...
EXTERN void clear_out_intermediate_points ( );
EXTERN int set_up_intermediate_course_for_tux ( );
...
EXTERN void AnalyzePlayersMouseClick ( ) ;
...
EXTERN void clear_tux_mission_info ( ) ;
...
EXTERN void MoveLevelDoors ( ) ;
EXTERN void WorkLevelGuns ( ) ;
...
EXTERN void CheckForTriggeredEventsAndStatements ( );
