...
static void print_menu_text(char *InitialText, char *MenuTexts[], int first_menu_item_pos_y, const char *background_name, void *MenuFont)
{
	...
	if (!strcmp(MenuTexts[0], SINGLE_PLAYER_STRING)) {
		RightPutStringFont(Screen, FPS_Display_BFont, GameConfig.screen_height - 1 * FontHeight(GetCurrentFont()), freedroid_version);
		// printf ("\n%s %s  \n", PACKAGE, VERSION);
...
#endif
		LeftPutStringFont(Screen, FPS_Display_BFont, GameConfig.screen_height - 2 * FontHeight(GetCurrentFont()), open_gl_string);
		sprintf(open_gl_string, _("OpenGL output active: "));
		if (use_open_gl)
			strcat(open_gl_string, _(" YES "));
		else
			strcat(open_gl_string, _(" NO "));
		LeftPutStringFont(Screen, FPS_Display_BFont, GameConfig.screen_height - FontHeight(GetCurrentFont()), open_gl_string);
	}
