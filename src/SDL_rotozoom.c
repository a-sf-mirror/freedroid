...
SDL_Surface *zoomSurface(SDL_Surface * src, double zoomx, double zoomy, int smooth)
{
	...
	if (is32bit) {
		...
		rz_dst =
		    SDL_CreateRGBSurface(SDL_SWSURFACE, dstwidth, dstheight, 32,
					 rz_src->format->Rmask, rz_src->format->Gmask, rz_src->format->Bmask, rz_src->format->Amask);
		if (rz_dst == NULL) {
	...
	if (src->flags & SDL_SRCCOLORKEY) {
		rz_dst->flags = rz_dst->flags | SDL_SRCCOLORKEY;

		rz_dst->format->colorkey = src->format->colorkey;
	}
