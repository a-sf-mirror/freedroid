...
static void handle_rectangle_floor ()
{
    ...
    if (calc_euklid_distance(mouse_mapcoord.x, mouse_mapcoord.y,
		state.r_start.x + state.r_len_x,
		state.r_start.y + state.r_len_y) > 0.5)
	{
	...
	for (i = state.r_start.x;
		...
		i += state.r_step_x)
	    {
	    for (j = state.r_start.y;
		    j != state.r_start.y + state.r_len_y + state.r_step_y;
		    j += state.r_step_y)
		{
...
static void start_line_walls(int windex)
{
    ...
    state.l_elements.position.x = (int)mouse_mapcoord.x +
	((obstacle_map[state.l_id].flags & IS_HORIZONTAL) ? 0.5 : 0);

    state.l_elements.position.y = (int)mouse_mapcoord.y + 
	((obstacle_map[state.l_id].flags & IS_HORIZONTAL) ? 0 : 0.5);

    state.l_elements.address = action_create_obstacle_user (EditLevel(), 
	    state.l_elements.position.x, state.l_elements.position.y, 
	    state.l_id);
...
{ 
	...
	wall->address = action_create_obstacle_user (EditLevel(),
		wall->position.x, wall->position.y, state.l_id);
    ...
    if ( (state.l_direction == (- actual_direction)) && 
	    (!list_empty(&(state.l_elements.list))) ) {
