...
static void get_excpt_list( char *section_pointer )
{
	...
	while ( ( ptr = strstr( ptr, "Rule=") ) != NULL )
	{
		...
		*end_string = '\0';
		...
		{
			if ( one_validator->initial != validator_type[0] ) continue;
			if ( one_validator->parse_excpt == NULL ) continue;
			...
			if ( item->opaque_data == NULL)	free(item);
			else list_add(&(item->node), &(one_validator->excpt_list));
		...
		if ( one_validator->initial == '\0' )
		{	// Sub-validator was not found
		...
		if ( tmp == '\0' ) break; // We are at the end of the section
...
static void *lvlval_chest_parse_excpt( char *string )
{
	struct chest_excpt_data *data = 
		(struct chest_excpt_data*)malloc(sizeof(struct chest_excpt_data));
...
#	define DIST_EPSILON 0.01f
	...
	if ( data1->obj_id != data2->obj_id ) return FALSE;
	if ( data1->obj_pos.z != data2->obj_pos.z ) return FALSE;
	
	float dist = calc_euklid_distance(data1->obj_pos.x, data1->obj_pos.y, data2->obj_pos.x, data2->obj_pos.y);
	if ( dist > DIST_EPSILON ) return FALSE;
	...
	for ( y_tile = 0 ; y_tile < validator_ctx->this_level->ylen ; ++y_tile )
	{
		for ( x_tile = 0 ; x_tile < validator_ctx->this_level->xlen ; ++x_tile )
		{
			for ( glue_index = 0 ; glue_index < MAX_OBSTACLES_GLUED_TO_ONE_MAP_TILE ; ++glue_index )
			{
				...
				if ( lookup_exception( this, &to_check ) ) continue;
				...
				if ( !SinglePointColldet(this_obs->pos.x, this_obs->pos.y, validator_ctx->this_level->levelnum, &filter) )
				{
					...
					printf( "[Type=\"C\"] Obj Idx=%d (X=%f:Y=%f:L=%d) : %s\n", obs_index, this_obs->pos.x, this_obs->pos.y,validator_ctx->this_level->levelnum, obstacle_map[this_obs->type].obstacle_short_name );
				}
			}
		}
	}
...
static void *lvlval_waypoint_parse_excpt( char *string )
{
	...
	struct waypoint_excpt_data *data = 
		(struct waypoint_excpt_data*)malloc(sizeof(struct waypoint_excpt_data));
...
#	define DIST_EPSILON 0.01f
	...
	if ( data1->subtest != data2->subtest) return FALSE;
	
	int i;
	for ( i=0; i<2; ++i )
	{
		if ( data1->wp_id[i] != data2->wp_id[i] ) return FALSE;
		if ( data1->wp_pos[i].z != data2->wp_pos[i].z ) return FALSE;
...
#	define MIN_DIST 1.0
	...
	{
		struct waypoint_excpt_data to_check = 
		    { 'P',
		      { i, 0 }, 
		      { 
		...
		if ( !SinglePointColldet(validator_ctx->this_level->AllWaypoints[i].x + 0.5, validator_ctx->this_level->AllWaypoints[i].y + 0.5, validator_ctx->this_level->levelnum, &WalkablePassFilter) )
		{
			if ( !pos_is_invalid )
			{	// First error : print header
			...
			printf( "[Type=\"WP\"] WP Idx=%d (X=%f:Y=%f:L=%d)\n", i, validator_ctx->this_level->AllWaypoints[i].x + 0.5, validator_ctx->this_level->AllWaypoints[i].y + 0.5, validator_ctx->this_level->levelnum);
		}
	}
	...
	for ( i = 0; i < validator_ctx->this_level->num_waypoints; ++i )
	{
		...
		if ( !lookup_exception( this, &to_check ) && 
		     validator_ctx->this_level->AllWaypoints[i].num_connections == 0 )
		{
			if ( !conn_is_invalid )
			{	// First error : print header
	...
	{
		gps wp_i = { validator_ctx->this_level->AllWaypoints[i].x + 0.5, validator_ctx->this_level->AllWaypoints[i].y + 0.5,  validator_ctx->this_level->levelnum };
		...
		{			
			...
			gps wp_j = { validator_ctx->this_level->AllWaypoints[j].x + 0.5, validator_ctx->this_level->AllWaypoints[j].y + 0.5,  validator_ctx->this_level->levelnum };
			...
			if ( dist < MIN_DIST ) {
				...
				printf( "[Type=\"WD\"] WP1 Idx1=%d (X1=%f:Y1=%f:L1=%d) <-> WP2 Idx2=%d (X2=%f:Y2=%f:L2=%d) : distance = %.3f\n", 
				        i, wp_i.x, wp_i.y, validator_ctx->this_level->levelnum,
				        j, wp_j.x, wp_j.y, validator_ctx->this_level->levelnum,
				        dist);
			}
		}
	}
	...
	for ( i = 0; i < validator_ctx->this_level->num_waypoints; ++i )
	{
		if ( validator_ctx->this_level->AllWaypoints[i].num_connections == 0 ) continue;
		
		gps from_pos = { validator_ctx->this_level->AllWaypoints[i].x + 0.5, validator_ctx->this_level->AllWaypoints[i].y + 0.5,  validator_ctx->this_level->levelnum };
		...
		for ( j = 0; j < validator_ctx->this_level->AllWaypoints[i].num_connections; ++j )
		{
			...
			gps to_pos   = { validator_ctx->this_level->AllWaypoints[wp].x + 0.5, validator_ctx->this_level->AllWaypoints[wp].y + 0.5, validator_ctx->this_level->levelnum };
...
static void *lvlval_jumptarget_parse_excpt( char *string )
{
	struct jumptarget_excpt_data *data = 
		(struct jumptarget_excpt_data*)malloc(sizeof(struct jumptarget_excpt_data));
...
static int lvlval_jumptarget_cmp_data( void *opaque_data1, void *opaque_data2 )
{
	...
	if ( data1->jumptarget != data2->jumptarget ) return FALSE;
	if ( data1->from_level != data2->from_level ) return FALSE;
	if ( data1->to_level != data2->to_level ) return FALSE;
...
static int lvlval_jumptarget_execute( struct level_validator *this, struct lvlval_ctx *validator_ctx ) 
{
	...
	if ( validator_ctx->this_level->jump_target_north != -1 && curShip.AllLevels[validator_ctx->this_level->jump_target_north] == NULL)
	{
		...
		if ( !lookup_exception( this, &to_check ) )
		{
			validator_print_header( validator_ctx, "Non existant jump target", "The north jump target on a level points to a non existing level.");
			is_invalid = TRUE;
	...
	if ( validator_ctx->this_level->jump_target_west != -1 && curShip.AllLevels[validator_ctx->this_level->jump_target_west] == NULL) 
	{
		...
		if ( !lookup_exception( this, &to_check ) )
		{
			validator_print_header( validator_ctx, "Non existant jump target", "The west jump target on a level points to a non existing level.");
			is_invalid = TRUE;
	...
	if ( validator_ctx->this_level->jump_target_east != -1 && curShip.AllLevels[validator_ctx->this_level->jump_target_east] == NULL) 
	{
		...
		if ( !lookup_exception( this, &to_check ) )
		{
			validator_print_header( validator_ctx, "Non existant jump target", "The east jump target on a level points to a non existing level.");
			is_invalid = TRUE;
	...
	if ( validator_ctx->this_level->jump_target_south != -1 && curShip.AllLevels[validator_ctx->this_level->jump_target_south] == NULL) 
	{
		...
		if ( !lookup_exception( this, &to_check ) )
		{
			validator_print_header( validator_ctx, "Non existant jump target", "The south jump target on a level points to a non existing level.");
			is_invalid = TRUE;
...
static void *lvlval_interface_parse_excpt( char *string )
{
	struct interface_excpt_data *data = 
		(struct interface_excpt_data*)malloc(sizeof(struct interface_excpt_data));
...
static int lvlval_interface_execute( struct level_validator *this, struct lvlval_ctx *validator_ctx )
{
	...
	for ( y_tile = 0 ; y_tile < validator_ctx->this_level->ylen ; ++y_tile )
	{
		for ( x_tile = 0 ; x_tile < validator_ctx->this_level->xlen ; ++x_tile )
		{		
			if ( ( y_tile >= validator_ctx->this_level->jump_threshold_north ) && ( y_tile <= (validator_ctx->this_level->ylen - validator_ctx->this_level->jump_threshold_south) ) &&
			     ( x_tile >= validator_ctx->this_level->jump_threshold_west  ) && ( x_tile <= (validator_ctx->this_level->xlen - validator_ctx->this_level->jump_threshold_east ) ) )
				continue;
...
void LevelValidation()
{
	...
	for ( l = 0; l < curShip.num_levels; ++l )
	{
		...
		else
		{
			...
			while ( one_validator = &(level_validators[v++]),  one_validator->execute != NULL ) level_is_invalid |= one_validator->execute(one_validator, &validator_ctx);
