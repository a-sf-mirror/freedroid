...
move_all_items_in_area(Level source_level,
		       ...
		       float source_area_width, float source_area_height, Level target_level, float target_start_x, float target_start_y)
{
	int i;
	int j;

	for (i = 0; i < MAX_ITEMS_PER_LEVEL; i++) {
		if (source_level->ItemList[i].type <= (-1))
			continue;
		if (source_level->ItemList[i].pos.x < source_start_x)
			continue;
		if (source_level->ItemList[i].pos.y < source_start_y)
			continue;
		if (source_level->ItemList[i].pos.x > source_start_x + source_area_width)
			continue;
		if (source_level->ItemList[i].pos.y > source_start_y + source_area_height)
			continue;
		...
		for (j = 0; j < MAX_ITEMS_PER_LEVEL; j++) {
			if (target_level->ItemList[j].type == (-1))
				break;
		}
		if (j >= MAX_ITEMS_PER_LEVEL) {
			ErrorMessage(__FUNCTION__, "No more free item entries in target level!", PLEASE_INFORM, IS_FATAL);
		}
 ...
 */
void move_all_items_to_level(int target_level)
{
	...
	if (source_level_num != (-1)) {
		...
		move_all_items_in_area(curShip.AllLevels[source_level_num], 0, curShip.AllLevels[source_level_num]->ylen - AreaHeight,
				       AreaWidth, AreaHeight, curShip.AllLevels[target_level], 0, 0);
	...
	if (source_level_num != (-1)) {
		...
		move_all_items_in_area(curShip.AllLevels[source_level_num], 0, 0,
				       ...
				       curShip.AllLevels[target_level], 0, curShip.AllLevels[target_level]->ylen - AreaHeight);
	...
	if (source_level_num != (-1)) {
		...
		move_all_items_in_area(curShip.AllLevels[source_level_num], 0, 0,
				       AreaWidth, AreaHeight,
				       curShip.AllLevels[target_level], curShip.AllLevels[target_level]->xlen - AreaWidth, 0);
