 ...
 * ---------------------------------------------------------------------- */
void 
InitiateNewCharacter (int CharacterClass )
{
    int i;
	...
	Me . Inventory [ i ] . inventory_position.x = -1 ;
	Me . Inventory [ i ] . inventory_position.y = -1 ; 
	...
	case WAR_BOT:
	    ...
	    Me .drive_item.type = ITEM_ANTIGRAV_POD ;
	    ...
	    FillInItemProperties ( & Me .Inventory[ 0 ] , TRUE , 0 , 1);
	    FillInItemProperties ( & Me .Inventory[ 1 ] , TRUE , 0 , 1);
	    FillInItemProperties ( & Me .Inventory[ 2 ] , TRUE , 0 , 1);
	    FillInItemProperties ( & Me .Inventory[ 3 ] , TRUE , 0 , 1);
	    break;
	...
	case SNIPER_BOT: 
	    ...
	    Me .drive_item.type = ITEM_ANTIGRAV_POD;
	    Me .Inventory[ 0 ].type = ITEM_LASER_PISTOL;
	    Me .Inventory[ 0 ].inventory_position.x = 0;
	    Me .Inventory[ 0 ].inventory_position.y = 0;
    ...
    FillInItemProperties ( & ( Me .weapon_item ) , TRUE , 0 , 1);
    FillInItemProperties ( & ( Me .drive_item ) , TRUE , 0 , 1);
...
}; // void InitiateNewCharacter ( int CharacterClass )
...
/*  if ( BonusItem->type == ITEM_START_PLUGIN_WARRIOR )
    {
      Me . freezing_melee_targets = 7 ;
    }
  if ( BonusItem->type == ITEM_START_PLUGIN_SNIPER )
    {
      Me . double_ranged_damage = TRUE ;
    }
