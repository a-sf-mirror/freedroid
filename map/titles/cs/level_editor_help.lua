...
Pro změnu současné úrovně klikněte na sousední číslo úrovně v pravém dolním rohu minimapy nebo vyberte požadovanou úroveň z menu editoru (popsáno později).
    ...
    --- Editace map ---
Existují čtyři módy editace: Editace překážek, editace podlady, editace předmětů a editace navigačních bodů.

Zvolené tlačítko v levém dolním rohu ukazuje objekty, které lze vybrat nebo vložit.
Když je tlačítko vybráné a nacházíte se v režimu umístění, objekt, který bude umístěn, je zvýrazněn v pásovém menu horní části obrazovky. Výběr v pásu karet je rozdělen záložkami ihned pod ním.
...
Stisknutím mezerníku vstoupíte do režimu výběru, který je znázorněn změnou kurzoru. Můžete si vybrat pouze skupinu objektů reprezentovaných aktuálně aktivovním režimem objektů.
...
Za účelem výběru tohoto režimu, klikněte na tlačítko, které říká, že "Překážka" ve výběru kategorií v levé dolní části.
...
Klikněte na nejvíce vlevo umístěné tlačítko z pětice tlačítek (ukazuje malou mřížku) nad výběrem kategorie, aby došlo k zobrazení mřížky s čísly. Použijte levé tlačítko pro zapnutí a vypnutí mřížky a pravé tlačítko myši pro změnu režimu mřížky.
Tato čísla se vztahují k číslům na vaší numerické klávesnici, pokud ji máte. Stisk tlačítka "1" bude pokládat překážku, která je zvýrazněná ve výběru objektů na pozici číslice "1" na fialové roštu.
...
Symbol s přeškrtnutou stopou není ve skutečnosti objekt, ale pouze neviditelná oblast blokování ("kolizní obdélník"). Kolizní obdélníky jsou samotným jádrem každého objektu, protože zabraňují procházení skrze ně, jak to možné v případě navigačních bodů a podlahových dlaždic.
...
Můžete také vyříznout (Ctrl+x lze také použít ke smazání překážek, pokud je opětovně nevložíte ;) ), kopírovat (Ctrl+c) a vkládat (Ctrl+v) vyříznuté nebo zkopírované překážky.
...
Jednodušše vyberte požadovanou truhlu a klikněte na tlačítko úplně vlevo v horní řadě tlačítek.
...
Nejsou zde žádné speciální podlahy, jen s obyčejnou dekorací.
...
Vyberte mód předmětů a klikněte na předmět zobrazení ve výběru předmětů. U některých předmětů je nutné zadat počet před tím, než jsou umístěny.
...
Štítky map jsou používány k definování pozic nehrajících postav (viz ReturnOfTux.droids], událostí, ke kterým dochází při pohybu Tux okolo těchto postav (viz events.dat) nebo míst používaných pro pohyb nehrajících osob pomocí skriptu lua (události, úkoly a dialogy).
...
Štítky překážek jsou důležité, takže některé překážky lze označit uskutečněním událostí (například v průběhu plnění úkolu). Pokud má například nějaká událost vyjmout speciální překážku zdi, pak musí mít tato překážka nejprve jméno nebo ID, takže se na ni lze později odvolávat v popisu události.
Můžete je také použít pro přiřazení dialogů k překážkám, takže s nimi můžete mluvit, jako by byly nehrající postavy.
Pro vložení štítku k překážce ji nejprve musíte označit (viz vysvětlení módu překážek výše).
...
Abyste uložili mapu, klikněte na malou ikonu disku v pravé horní části obrazovky editoru. Ikona dveří umožňuje ukončení editor.
...
Obecné rady:
	...
	Získání přehledu
Chcete-li změnit faktor zvětšení, klikněte klávesu "o" nebo klikněte na ikonu s lupou.
Zkuste levé a pravé kliknutí, abyste získali různé faktory zvětšení.


	Menu editoru

K menu lze přistupovat stisknutím ESC.

		"Úroveň:"
Zde můžete snadno přejít do ostatních úrovní. Můžete použít buď klávesy šipek, které mají tuto možnost volenou tak,
aby umožnily přechod na další nebo předchozí úroveň (odkazují na čísla úrovní) nebo na ni klepnete, zadejte číslo požadované úrovně a stiskněte tlačítko Enter.

		Možnosti úrovně
				Úroveň:⇥Viz vysvětlení výše
				Jméno:⇥Jméno mapy zobrazené u GPS v pravém horním rohu herní obrazovky. Zobrazeni GPS ve hře lze zakázat pomocí nabídky možností.
				Velikost:⇥Můžete zvýšit nebo snížit velikost vaší úrovně. Vyberte požadovanou hranu, kam chcete přidat/odebrat řadu dlaždic a klikněte na tlačítko <- nebo -> tlačítka se šipkami.
				Podlahové vrstvy: Chcete-li změnit počet podlahových vrstev na aktuální úrovni, použijte <- nebo -> tlačítka se šipkami.
				Rozhraní okraje:⇥Zde lze nastavit úrovně, které mají sousedit s aktuální úrovní.  Zadejte číslo úrovně pro příslušný okraj.
								Úroveň může mít pouze jednu sousední úroveň (dotýkající se okrajem) v každém ze čtyř hlavních směrů (Sever, Jih, Západ, Východ).
				Náhodná kobka:⇥Pokud tuto volbu nastavíte na "Ano", vygeneruje mapa automaticky podzemní bludiště. Kliknutím na volby nastavíte počet teleportů do a z této mapy.
				...
				Blokování teleportu:⇥Učiňte teleportaci pryč z úrovně (ne)možnou.
				...
				Světlo:⇥⇥⇥Jak moc světla byste chtěli? Stiskněte mezerník pro přepnutí mezi módy okolí (obecný jas aktuální mapy) a bonusu (světlo vyzařované nějakou překážkou, jako jsou lampy nebo houby).
...
n⇥⇥⇥⇥⇥⇥procházení skrze vybrané překážky (další)
z⇥⇥⇥⇥⇥⇥vrátit zpět poslední akci
y⇥⇥⇥⇥⇥⇥obnovit poslední akci
c⇥⇥⇥⇥⇥⇥nastavení cesty mezi navigačními body
ctrl+x nebo backspace⇥⇥vyřízkout vybraný(é) objekt(y), lze použít k mazání předmětů jejich opětovným nevložením.
...
Pokud se setkáte s problémy s editorem, prosím, kontaktujte nás.
